...
export class AppGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    handleJoinQueue(@ConnectedSocket() client: Socket) {
        ...
        if (this.socketToRoom.has(client.id) || this.waitingQueue.includes(client.id)) {
            return;
        }
        ...
        if (this.waitingQueue.length > 0) {
            ...
            if (partnerId) {
                ...
                // Join both
                client.join(roomId);
                this.server.sockets.sockets.get(partnerId)?.join(roomId);
    ...
    handleSignal(
        ...
        if (roomId) {
            ...
            client.to(roomId).emit('signal', { sender: client.id, signal: payload.signal });
        }
    }
    ...
    handleMessage(
        ...
        const roomId = this.socketToRoom.get(client.id);
        if (roomId) {
            client.to(roomId).emit('receive_message', { sender: 'partner', text: payload.text });
